...
void CPlaneWarView::OnTimer(UINT_PTR nIDEvent)
{
	...
	*/
	...
	if (isStarted == FALSE) {
		...
		cdc.TextOutW(rect.right/2-120, 210, _T("方向控制：鼠标、方向键、ASDW"));
		cdc.TextOutW(rect.right/ 2 - 120, 225, _T("射击：鼠标左键、空格键"));
		...
		cdc.TextOutW(rect.right / 2 - 120, 330, _T("初始能量值：0"));
		...
		cdc.TextOutW(rect.right / 2 - 120, 375, _T("能量值随着游戏进程增加，可通过使用能量值使用防护罩、战机升级、战机大招的使用。"));
   ...
   //（2）添加爆炸效果，敌机炸弹打中战机
	...
	if (myplane != NULL)
	{
		...
		cdc.TextOutW(rect.right - 12 * DEFAULT_LIFE - 45, 15, _T("能量："));
		CBrush magicBrush;
	...
	if (isStop == FLAG_RESTART) {
		...
		cdc.SetBkMode(TRANSPARENT);
		cdc.SetTextColor(RGB(255, 255, 255));
		cdc.TextOutW(rect.right/2-150, rect.bottom/2-30, _T("恭喜通关！是否重新开始？Y/N"));
	}
		...
		cdc.TextOutW(rect.right / 2 - 100, rect.bottom / 2 +10, _T("重新开始？Y/N"));
	}
